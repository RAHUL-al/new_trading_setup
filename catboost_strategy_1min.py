"""
catboost_strategy_1min.py — CatBoost ML Strategy for NIFTY (1-Minute ONLY)

Uses ONLY 1-minute OHLCV data to train a CatBoost model.
No 2-minute features are used.

Usage:
    python catboost_strategy_1min.py
    python catboost_strategy_1min.py --min-atr 6.5
"""
...
# No 2-min features in this version
...
def main():
    ...
    print(f"\n🤖 CATBOOST ML STRATEGY — 1-MIN ONLY")
    ...
    # ── Build features (1-min ONLY) ──
    print(f"\n📊 Building 1-min features ONLY...")
    features = build_features_1min(df_1m)
    print(f"  Total features: {features.shape[1]} columns (1-min only)")
    ...
    if all_trades:
        ...
        trades_df.to_csv("catboost_trades_1min.csv", index=False)
        print(f"\n💾 Trades saved: catboost_trades_1min.csv")

    # Save model
    model.save_model("catboost_nifty_model_1min.cbm")
    print(f"💾 Model saved: catboost_nifty_model_1min.cbm")
