...
POSITIONS_KEY = "active_positions_atr"
TRADE_HISTORY_KEY = f"trade_history_atr_{datetime.now().date()}"
TRADING_SYMBOLS_KEY = "Trading_symbol"
CANDLE_CACHE_KEY = "candle:last"
TRAILING_DATA_KEY = "trailing_data_atr"

CHAN_PRICE_PREFIX = "price_atr:"
CHAN_SIGNAL_BUY = "signal_atr:buy"
CHAN_SIGNAL_SELL = "signal_atr:sell"
CHAN_CANDLE_CLOSE = "candle_atr:close"
...
class TradingBot:
    ...
    async def take_buy(self, quantity: int = 1, signal_candle: Optional[CandleData] = None) -> bool:
        ...
        if current_time < self.market_denied_position.time():
            ...
            self.open_pos = pos
            await self.save_position(pos)
            logger.info(f"BUY CE @ {price} Initial SL {initial_sl} (ATR: {current_atr:.2f}, Multiplier: {atr_multiplier:.2f})")
            return True
    ...
    async def take_sell(self, quantity: int = 1, signal_candle: Optional[CandleData] = None) -> bool:
        ...
        current_time = now.time()
        if current_time < self.market_denied_position.time():
            print("coming inside the funtion take sell ------------------------------> : ")
            if not await self.is_atr_above_threshold():
            ...
            print("this is inital stoploss inside the take sell : ",initial_sl)
            ...
            self.open_pos = pos
            await self.save_position(pos)
            logger.info(f"SELL PE @ {price} Initial SL {initial_sl} (ATR: {current_atr:.2f}, Multiplier: {atr_multiplier:.2f})")
            return True
    ...
    # ---------- main ----------
    async def run(self):
        logger.info("TradingBot starting with dynamic ATR-based trailing stop loss…")
