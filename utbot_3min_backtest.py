"""
utbot_3min_backtest.py — UT Bot Alert Strategy Backtest (NIFTY 3-min)

STRATEGY: UT Bot Alert (Pure ATR + Trailing Stop) on 3-MINUTE timeframe
...
DATA: NIFTY 3-minute OHLCV CSV

Usage:
    python utbot_3min_backtest.py
    python utbot_3min_backtest.py --file nifty_3min_data.csv
    python utbot_3min_backtest.py --atr-key 1.5 --min-atr 8
    python utbot_3min_backtest.py --atr-period 10 --atr-key 1.2
    python utbot_3min_backtest.py --window-start 13:00 --window-end 15:03
"""
...
def compute_ut_bot_signals(df, atr_period=14, key_value=1.0, min_atr=6.5):
    """
    UT Bot Alert signal generation on 3-min candles.
...
def print_detailed_trades(all_trades, daily_results,
                          log_file="utbot_3min_detailed_log.txt"):
...
def print_summary(all_trades, daily_results):
    ...
    # ── Print ──
    print(f"\n{'='*70}")
    print(f"  🤖 UT BOT ALERT (3-MIN) — OVERALL RESULTS")
    print(f"{'='*70}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="UT Bot Alert Backtest (NIFTY 3-min)"
    )
    parser.add_argument("--file", default="nifty_3min_data.csv",
                        help="3-min data CSV file (default: nifty_3min_data.csv)")
    ...
    try:
        ...
        print(f"Make sure you have the 3-min NIFTY data CSV.")
        return
    ...
    print(f"\n{'='*70}")
    print(f"  🤖 UT BOT ALERT BACKTEST — 3-MINUTE TIMEFRAME")
    print(f"{'='*70}")
    ...
    print(f"\n⚡ Computing UT Bot signals on 3-min candles...")
    ...
    trades_df.to_csv("utbot_3min_trades.csv", index=False)
    print(f"\n💾 Trades saved: utbot_3min_trades.csv")
