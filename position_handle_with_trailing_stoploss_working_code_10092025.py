...
class TradingBot:
    ...
    async def take_buy(self, quantity: int = 1, signal_candle: Optional[CandleData] = None) -> bool:
        ...
        print("this is signal data : ",signal_candle)
        pos = Position(
            ...
            highest_close=signal_candle.close,  # Initialize with entry price
        )
        self.open_pos = pos
        await self.save_position(pos)
    ...
    async def take_sell(self, quantity: int = 1, signal_candle: Optional[CandleData] = None) -> bool:
        ...
        pos = Position(
            ...
            entry_time=datetime.now().isoformat(),
            stop_loss=initial_sl,
            lowest_close=signal_candle.close,  # Initialize with entry price
        )
        self.open_pos = pos
        await self.save_position(pos)
