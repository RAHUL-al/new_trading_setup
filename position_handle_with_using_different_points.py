...
POINTS_TRACKER_KEY = "points_tracker_points"
...
# Points-based position sizing configuration
POINTS_QUANTITY_MAPPING = [
    (-8, 2),   # -8 points: 2x quantity
    (-20, 3),  # -20 points: 3x quantity  
    (-40, 4),  # -40 points: 4x quantity
    (-60, 5),  # -60 points: 5x quantity
]
DEFAULT_QUANTITY = 1
...
class Position:
    ...
    recovery_target_per_share: float = 0.0  # Points needed per share to recover
...
class PointsTracker:
    ...
    current_quantity_level: int = 1  # 1x, 2x, 3x, etc.
    recovery_points_needed: float = 0.0  # Points needed to fully recover
    is_in_recovery: bool = False
    last_reset_time: str = field(default_factory=lambda: datetime.now().isoformat())
...
class TradingBot:
    def __init__(self):
        ...
        self.points_tracker: PointsTracker = PointsTracker()
    ...
    async def load_points_tracker(self):
        """Load points tracker from Redis"""
        try:
            data = await self.r.get(POINTS_TRACKER_KEY)
            if data:
                points_data = json.loads(data)
                self.points_tracker = PointsTracker(
                    total_points=float(points_data.get("total_points", 0.0)),
                    current_quantity_level=int(points_data.get("current_quantity_level", 1)),
                    recovery_points_needed=float(points_data.get("recovery_points_needed", 0.0)),
                    is_in_recovery=bool(points_data.get("is_in_recovery", False)),
                    last_reset_time=points_data.get("last_reset_time", datetime.now().isoformat())
                )
        except Exception as e:
            logger.error(f"Error loading points tracker: {e}")
            self.points_tracker = PointsTracker()

    async def save_points_tracker(self):
        """Save points tracker to Redis"""
        try:
            await self.r.set(
                POINTS_TRACKER_KEY, 
                json.dumps(asdict(self.points_tracker)),
                ex=86400
            )
        except Exception as e:
            logger.error(f"Error saving points tracker: {e}")

    def calculate_quantity_based_on_points(self) -> int:
        """Calculate quantity based on current negative points"""
        current_points = self.points_tracker.total_points
        
        # If we're positive or at zero, use default quantity
        if current_points >= 0:
            self.points_tracker.current_quantity_level = 1
            self.points_tracker.is_in_recovery = False
            self.points_tracker.recovery_points_needed = 0.0
            return DEFAULT_QUANTITY
        
        # Sort thresholds in ascending order (most negative first)
        sorted_mapping = sorted(POINTS_QUANTITY_MAPPING, key=lambda x: x[0])
        
        for threshold, quantity in sorted_mapping:
            if current_points <= threshold:
                self.points_tracker.current_quantity_level = quantity
                # Calculate recovery points needed: divide total negative points by quantity
                self.points_tracker.recovery_points_needed = abs(current_points) / quantity
                self.points_tracker.is_in_recovery = True
                return quantity
        
        # If no threshold met but still negative, use default
        self.points_tracker.current_quantity_level = 1
        self.points_tracker.recovery_points_needed = abs(current_points)
        self.points_tracker.is_in_recovery = True
        return DEFAULT_QUANTITY

    async def update_points_after_trade(self, pnl_points: float):
        """Update points tracker after a trade with CORRECT calculation"""
        old_points = self.points_tracker.total_points
        self.points_tracker.total_points += pnl_points
        
        logger.info(f"Points updated: {old_points:.2f} -> {self.points_tracker.total_points:.2f} (Trade PnL: {pnl_points:.2f})")
        
        # Reset recovery if we become positive
        if self.points_tracker.total_points >= 0:
            self.points_tracker.is_in_recovery = False
            self.points_tracker.recovery_points_needed = 0.0
            self.points_tracker.current_quantity_level = 1
            logger.info("✅ Recovery completed! Points reset to positive/zero")
        
        await self.save_points_tracker()

    async def check_recovery_exit(self, current_price: float, position: Position) -> bool:
        """Check if we should exit for recovery purposes - CORRECT LOGIC"""
        if not self.points_tracker.is_in_recovery or not position.recovery_mode:
            return False
        
        # Calculate P&L per share for this position
        if position.position_type == "BUY":
            pnl_per_share = current_price - position.entry_price
        else:
            pnl_per_share = current_price - position.entry_price
        
        # Check if we've reached recovery target per share
        if pnl_per_share >= position.recovery_target_per_share:
            await self.close_position("RECOVERY_TARGET")
            logger.info(f"🎯 Recovery target achieved: {pnl_per_share:.2f} points per share (Target: {position.recovery_target_per_share:.2f})")
            return True
            ...
            if await self.r.exists(POSITIONS_KEY):
                ...
                if positions:
                    ...
                    self.open_pos = Position(
                        ...
                        recovery_mode=bool(pdata.get("recovery_mode", False)),
                        recovery_target_per_share=float(pdata.get("recovery_target_per_share", 0.0)),
                    )
    ...
    async def update_trailing_stop_loss(self, candle: CandleData):
        ...
        if pos.option_type == "CE":
            if current_close > pos.highest_close:
                ...
                # Use dynamic ATR-based trailing stop
    ...
    async def take_buy(self, quantity: int = None, signal_candle: Optional[CandleData] = None) -> bool:
        """Take BUY position with dynamic quantity based on points"""
        if quantity is None:
            quantity = self.calculate_quantity_based_on_points()
            
        now = datetime.now()
        current_time = now.time()
        if current_time < self.market_denied_position.time():
            print("coming inside the take buy position  ---------------------------------------> : ")
            ...
            # Get current ATR value for position
            ...
            if signal_candle is None:
                ...
                if low is None:
                    ...
                    return False
                # Use ATR-based initial stop loss
                initial_sl = current_index - (current_atr * atr_multiplier)
            else:
                # Use ATR-based initial stop loss
                initial_sl = current_index - (current_atr * atr_multiplier)
                
            print("this is initial stoploss : ", initial_sl)
            
            # Set recovery target per share if in recovery mode
            recovery_mode = self.points_tracker.is_in_recovery
            recovery_target_per_share = self.points_tracker.recovery_points_needed if recovery_mode else 0.0
            ...
            pos = Position(
                ...
                recovery_target_per_share=recovery_target_per_share,
            )
            self.open_pos = pos
            await self.save_position(pos)
            
            points_info = f"Total Points: {self.points_tracker.total_points:.2f}, Quantity: {quantity}x"
            if recovery_mode:
                points_info += f" | Recovery Target: {recovery_target_per_share:.2f} points per share"
                
            logger.info(f"BUY CE @ {price} Qty: {quantity} Initial SL {initial_sl} (ATR: {current_atr:.2f}, Multiplier: {atr_multiplier:.2f}) | {points_info}")
            return True

    async def take_sell(self, quantity: int = None, signal_candle: Optional[CandleData] = None) -> bool:
        """Take SELL position with dynamic quantity based on points"""
        if quantity is None:
            quantity = self.calculate_quantity_based_on_points()
        ...
        current_time = now.time()
        if current_time < self.market_denied_position.time():
            print("coming inside the function take sell ------------------------------> : ")
            if not await self.is_atr_above_threshold():
            ...
            print("this is initial stoploss inside the take sell : ", initial_sl)
            ...
            logger.info(f"SELL PE @ {price} Qty: {quantity} Initial SL {initial_sl} (ATR: {current_atr:.2f}, Multiplier: {atr_multiplier:.2f}) | {points_info}")
            return True
    ...
    async def close_position(self, reason: str) -> bool:
        async with self._close_lock:
            ...
            # Calculate P&L in points - CORRECT CALCULATION
            if pos.position_type == "BUY":
                pnl_points = (cur_price - pos.entry_price) * pos.quantity
            else:
                pnl_points = (cur_price - pos.entry_price) * pos.quantity
            ...
            # Update points tracker with CORRECT calculation
            await self.update_points_after_trade(pnl_points)
            
            self.open_pos = None
            await self.save_trade(tr)
            await self.remove_position(token)
            logger.info(f"Closed {token} reason={reason} pnl_points={tr.pnl} total_points={self.points_tracker.total_points:.2f}")
            return True
        ...
        while True:
            try:
                if self.is_market_hours():
                    current_atr = await self.get_current_atr()
                    atr_multiplier = self.get_dynamic_atr_multiplier(current_atr)
                    # Log ATR value periodically
                    # logger.info(f"Current ATR: {current_atr:.2f}, Multiplier: {atr_multiplier:.2f}")
    ...
    async def handle_index_tick(self, price: float):
        ...
        # Check recovery exit first
        if await self.check_recovery_exit(price, pos):
            return
    ...
    async def on_buy_signal(self, signal_candle: Optional[CandleData] = None):
        ...
        if not self.open_pos:
            await self.take_buy(signal_candle=signal_candle)
    ...
    async def on_sell_signal(self, signal_candle: Optional[CandleData] = None):
        ...
        if not self.open_pos:
            await self.take_sell(signal_candle=signal_candle)
    ...
    async def task_square_off_scheduler(self):
        while True:
            ...
            if now.time() >= self.square_off_time.time() and self.is_market_hours():
                ...
                # Reset points tracker at market close
                self.points_tracker = PointsTracker()
                await self.save_points_tracker()
                logger.info("Points tracker reset for new trading day")
    ...
    # ---------- main ----------
    async def run(self):
        logger.info("TradingBot starting with dynamic ATR-based trailing stop loss and CORRECT points-based position sizing…")
        ...
        # Load points tracker
        await self.load_points_tracker()
        logger.info(f"Loaded points tracker: Total Points: {self.points_tracker.total_points:.2f}, Recovery Mode: {self.points_tracker.is_in_recovery}, Current Level: {self.points_tracker.current_quantity_level}x")
