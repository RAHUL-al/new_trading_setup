...
# NEGATIVE_8_THRESHOLD = -8
# NEGATIVE_20_THRESHOLD = -20
# NEGATIVE_40_THRESHOLD = -40
# NEGATIVE_60_THRESHOLD = -60
# PROFIT_TARGET = 35
...
#         """Calculate next quantity based on total points"""
#         if total_points >= NEGATIVE_60_THRESHOLD and total_points < NEGATIVE_40_THRESHOLD:
#             return 5
#         elif total_points >= NEGATIVE_40_THRESHOLD and total_points < NEGATIVE_20_THRESHOLD:
#             return 4
#         elif total_points >= NEGATIVE_20_THRESHOLD and total_points < NEGATIVE_8_THRESHOLD:
#             return 3
#         elif total_points >= NEGATIVE_8_THRESHOLD and total_points < 0:
#             return 2
#         else:
#             return 1
...
#         if self.pnl_tracker.total_points < 0:
#             self.pnl_tracker.recovery_mode = True
#             self.pnl_tracker.recovery_points = abs(self.pnl_tracker.total_points)
...
#             logger.warning(f"⚠️ RECOVERY MODE: Total Points={self.pnl_tracker.total_points:.2f}, "
#                           f"Next Quantity={self.pnl_tracker.current_quantity}x")
        
#         elif self.pnl_tracker.recovery_mode and self.pnl_tracker.total_points >= PARTIAL_EXIT_PROFIT:
#             # Exited recovery mode with profit
#             logger.info(f"✅ Recovery successful! Profit secured: {self.pnl_tracker.total_points:.2f}")
...
#         elif self.pnl_tracker.total_points > 0:
#             # Direct profit without recovery
#             if self.pnl_tracker.total_points >= PARTIAL_EXIT_PROFIT:
#                 self.pnl_tracker.secured_profit += self.pnl_tracker.total_points
#                 logger.info(f"💎 Profit secured: {self.pnl_tracker.total_points:.2f}")
#                 self.pnl_tracker.total_points = 0.0
#                 self.pnl_tracker.current_quantity = 1
#                 self.next_trade_quantity = 1
...
#         # Check if we have recovered minimum required points
#         required_recovery = abs(self.pnl_tracker.recovery_points) / position.quantity
        
#         if pnl_per_unit >= required_recovery:
#             # Exit one quantity
#             exit_quantity = 1
...
#             logger.info(f"🔄 Partial Exit: Closed {exit_quantity} unit(s), "
...
#                     # Don't take new positions but allow exits
                
#                 # Check for partial exits if in recovery mode
#                 if self.open_pos and self.pnl_tracker.recovery_mode:
...
#         logger.info("=" * 80)
#         logger.info("📋 Recovery Thresholds:")
#         logger.info(f"   -8 points → 2x quantity")
#         logger.info(f"   -20 points → 3x quantity")
#         logger.info(f"   -40 points → 4x quantity")
#         logger.info(f"   -60 points → 5x quantity")
#         logger.info(f"   +{PARTIAL_EXIT_PROFIT} points → Partial exit (1 unit)")
#         logger.info(f"   +{PROFIT_TARGET} secured profit → Stop trading")
#         logger.info("=" * 80)
