"""
catboost_strategy_2min.py — CatBoost ML Strategy for NIFTY (2-Minute ONLY)

Uses ONLY 2-minute OHLCV data. No 1-minute features.

Usage:
    python catboost_strategy_2min.py
    python catboost_strategy_2min.py --min-atr 6.5
"""
...
def build_features_2min(df):
    """Build features from 2-minute data directly."""
    ...
    features['body_pct_2m'] = (close - opn) / opn * 100
...
def main():
    ...
    parser.add_argument("--test-from", type=str, default="2026-01-01")
    ...
    # ── Load 2-min data ONLY ──
    print(f"Loading 2-min data: {args.file_2m}")
    try:
        df = pd.read_csv(args.file_2m)
    ...
    print(f"Date range: {df['Time'].iloc[0].strftime('%Y-%m-%d')} → {df['Time'].iloc[-1].strftime('%Y-%m-%d')}")

    print(f"\n🤖 CATBOOST ML STRATEGY — 2-MIN ONLY")
    ...
    # ── Build features (2-min ONLY) ──
    print(f"\n📊 Building 2-min features ONLY...")
    features = build_features_2min(df)
    print(f"  Total features: {features.shape[1]} columns (2-min only)")

    # ── Generate labels ──
    print(f"\n🏷️  Generating labels (lookahead={LOOKAHEAD}, threshold={THRESHOLD})...")
    labels = generate_labels(df, LOOKAHEAD, THRESHOLD)
    ...
    print(f"\n  Window candles: {window_mask.sum()} (of {len(df)})")
    ...
    full_predictions = np.zeros(len(df), dtype=int)
    ...
    print_results(all_trades, daily_results, f"TEST 2-MIN ({test_start} → {test_end})")
    
    # ── Detailed per-trade daily log ──
    print_detailed_daily_log(all_trades, daily_results, "catboost_detailed_log_2min.txt")

    # ── Save ──
    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df.to_csv("catboost_trades_2min.csv", index=False)
        print(f"\n💾 Trades saved: catboost_trades_2min.csv")

    # Save model
    model.save_model("catboost_nifty_model_2min.cbm")
    print(f"💾 Model saved: catboost_nifty_model_2min.cbm")
